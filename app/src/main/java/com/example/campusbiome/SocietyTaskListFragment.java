...
public class SocietyTaskListFragment extends Fragment {
    ...
    public static SocietyTaskListFragment newInstance(String societyId, String eventId, String eventTitle, String eventDate) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        eventFullDate = getArguments().getString("eventDate");
    ...
    private void toggleTaskStatus(SocietyTask task, String taskId) {
        String taskDateStr = task.getDueDay() + " " + task.getDueMonth() + " " + task.getDueYear() + " " + task.getDueTime() + " " + task.getDueAmPm();
        ...
        try {
            ...
            if (taskDate != null && taskDate.before(now)) {
                Toast.makeText(getContext(), "Cannot change status: This task is already in the past.", Toast.LENGTH_LONG).show();
                return;
            }
        } catch (Exception e) {
            Toast.makeText(getContext(), "Error validating task date.", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void showAddTaskDialog() {
        ...
        btnPublish.setOnClickListener(v -> {
            ...
            String taskDateStr = day + " " + month + " " + year + " " + time + " " + ampm;
            SimpleDateFormat sdf = new SimpleDateFormat("d MMMM yyyy h:mm a", Locale.getDefault());
            ...
            try {
                ...
                SimpleDateFormat eventSdf = new SimpleDateFormat("d MMMM yyyy", Locale.getDefault());
                Date eventDeadline = eventSdf.parse(eventFullDate);

                if (taskDate.before(now)) {
                    Toast.makeText(getContext(), "Task cannot be in the past!", Toast.LENGTH_SHORT).show();
                    return;
                }

                if (taskDate.after(eventDeadline)) {
                    Toast.makeText(getContext(), "Task cannot be after the event date!", Toast.LENGTH_SHORT).show();
                    return;
                }
    ...
    private void setupSpinners(Spinner spDay, Spinner spMonth, Spinner spYear, Spinner spAmPm, Spinner spPriority) {
